...
不圆满的例子全出在修 bug 的场景：Claude 三遍都先写了失败测试（上面那次单跑里看到的老习惯，这下三遍都对上了），Codex 有一遍越过“只重试一次，然后报告”这条规则接着重试，OpenCode 有一遍先写了失败测试。其余的每一遍都守住了。

规则很少单独摆在智能体面前，所以 `--context-file` 会在不带规则和带规则这两遍里，都把一份真实的项目文档放到规则前面。把本仓库自己的 `CLAUDE.md`（六十来行不相干的维护规则）压在上头之后，Claude 各跑两遍仍是 14/15（按遍数是 28/30），漏的还是同一处。其他语言包也一样：韩文、日文、中文三份档案，每个轴一个场景、各跑两遍，回来都是 7/8（按遍数是 14/16），漏的只有先写测试那一处，别的都没有。
...
| Claude Code, `~/.claude/CLAUDE.md` 里的一行 `@import` | 7/8 | 3 | 4 |
| gajae-code, `~/.gjc/agent/AGENTS.md` | 7/8 | 3 | 4 |
| OpenCode, `~/.config/opencode/AGENTS.md` | 6/8 | 2 | 5 |
| Kiro CLI, `~/.kiro/steering/xout.md` | 6/8 | 2 | 4 |

每个工具都读了自己的文件：每一行里都至少有两个答案，是因为区块在那儿才变的。不符的地方同样值得看。Claude 漏的那一处又是先写测试的老习惯；gajae-code 和 Kiro 在修 bug 时没先提方案就直接动手；Kiro 还在报错之后一直自己修；OpenCode 的两处不符，一处是一段没有选项字母的长篇散文，一处是先写测试的老习惯。Kiro 的文档说全局 steering 给 IDE 用、工作区 steering 给 CLI 用，可这里的 CLI 照样读了全局那份文件——探针就是用来发现这种事的。
...
默认运行器用 `--safe-mode` 启动 Claude Code，这个模式不加载你自己的 `CLAUDE.md`、插件和 MCP 服务器。没有它，"不带规则" 那一遍并不是真的不带规则：你的全局规则文件已经在里面，每次调用的开销也大约是三倍。本页的实测是在它成为默认值之前跑的，所以 "不带规则也一致" 一列混进了作者自己的规则；"规则保持" 各列不受影响。用 `--safe-mode` 重跑，一直不符的那一例就消失了：修 bug 场景每次都回到 test-after。先写测试这个答案从来不是模型的习惯，而是作者自己的 `~/.claude/CLAUDE.md` 里正好写着这句话，两遍都带着它。`--via-target claude` 是通过 `CLAUDE.md` 本身传递规则的测量，需要普通模式，`xout probe` 会拒绝这个组合。大批量时用 `--model` 指定小模型，答案只有一个字母。
...
一次会话量的是八个轴。你早就写好的 `CLAUDE.md` 里的行数远不止这些，其中大多数从没摆到智能体面前，看看它到底改不改得了什么。`xout audit` 把它们一行一行地量。

每一行，它都让你自己的智能体写一个小场景：一件事、一个照着这行做的下一步、一个违背这行的下一步；然后把这个场景问两遍，一遍就这么问，一遍把这一行当作常设指令放在前面。回来的答案会把这行归进四类之一：

- **本来就这么做** —— 没这行智能体也往那边走。这句话什么也没干，删掉也不会有变化。
- **这行起了作用** —— 不带它就走另一边，带上它就照办。规则本该是这个样子。
- **被无视** —— 这行就摆在智能体眼前，它照样反着来。这类最要紧：读起来像道防线，其实不是。
- **看不清** —— 没有能读出来的答案，或者几遍结果不一致。
...
标题、表格行、代码块和只有一个词的行，在送出去之前就被剔掉了，剩下的由 `--limit`（默认 60）卡住，所以第一遍跑得很便宜。等你看清第一遍说了什么，再往上调。

下面是拿写这个工具的人那份 187 行的全局 `CLAUDE.md` 跑的第一遍，运行器是 `claude -p`。36 行作为标题或代码被剔掉，头 60 行送了出去，智能体给其中 57 行写了场景（另外三行它判定不是指令）。这 57 行里：55 行本来就这么做，2 行这行起了作用，0 行被无视，0 行看不清。换句话说，一份用几个月手写出来的规则文件，里面几乎全是智能体本来就会做的事，而这一遍查下来，能指出是哪两句话在真正挣自己的位置。矛盾那部分才是有用的：一共四对，其中有 "always respond in Korean" 撞上 "ASCII output only"（韩文不是 ASCII），还有 "always end with a three-line summary" 撞上 "explain only when it is not obvious"。这两处谁都没注意到。回执里每个场景、每条回答都在。
...
| `xout audit [paths] --runner "…"` | 把你规则文件里已有的行一行一行地量：智能体本来就会这么做，还是这行改变了答案，又或者干脆被无视 | 自有目录（`audits/`） | 要手动开启 |
