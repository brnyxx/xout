...
<img src=".github/assets/logo.svg?v=2" alt="xout のロゴ: 深紅の線で 8 等分した円のうち、1 切れだけ黒く塗られている" width="96">
...
<img src=".github/assets/how-it-works.ja.gif?v=2" alt="3 つのパネル: 「バグを直して」に対する 2 つの振る舞いのうち間違ったほうが X で消され、6,561 通りのエージェント候補が 15 回の X で 1 つに絞られ、8 本のルールが 1 行の import を通じて CLAUDE.md に着地する" width="920">
...
問い 1 つにつき答え 1 つでは、測定として薄すぎます。そこで `--repeat` は同じ問いを何度か投げ、多数決で決めます。生の回答はすべてレシートに残ります。15 件すべてを 3 回ずつ、同じプロファイルで、同じ日に走らせました:

| ランナー | ルール維持 (多数決) | 3 回とも維持 | 維持した試行 | ルールが選択を動かした | ルールなしでも一致 |
|---|---|---|---|---|---|
...
きれいに揃わなかった件は、どれもバグ修正の場面です。Claude は 3 回とも失敗するテストを先に書きました (上の 1 回だけの記録で見えた癖が、これで 3 回とも確かめられたことになります)。Codex は一度、「1 回だけ再試行して、あとは報告する」というルールを越えて再試行を続けました。OpenCode は一度、失敗するテストを先に書きました。それ以外は毎回維持されています。

ルールがエージェントの前に単独で置かれることは、まずありません。そこで `--context-file` は、ルールなしの回にもルール付きの回にも、実在のプロジェクト文書をルールの前に置きます。このリポジトリ自身の `CLAUDE.md` (無関係な保守ルールが六十行ほど) を上に積んで埋めても、Claude はそれぞれ 2 回ずつ回して再び 14/15 を維持し (試行では 28/30)、外したのは同じ 1 件だけでした。ほかの言語パックも同じ挙動です。韓国語・日本語・中国語のプロファイルを、軸ごとに 1 場面、2 回ずつ回したところ、いずれも 7/8 (試行では 14/16) で、外したのはテストを先に書くあの 1 件だけでした。

ルールをプロンプトに入れて問うのは、ルールが効きうると示すところまでです。xout が書き込んだファイルをツールが実際に読んでいるかは別の問いなので、`xout probe --via-target codex` はプロンプトからルールを完全に外します。ルールなしの回では `~/.codex/AGENTS.md` から xout のブロックを抜き、ルール付きの回ではブロックを戻し、終わったらファイルを元どおりにします (先にセーブポイントを取ります)。同じプロファイル、軸ごとに 1 場面:

| ツールと、そのツール自身のルールファイル | ルール維持 | ルールが選択を動かした | ルールなしでも一致 |
|---|---|---|---|
| Codex CLI, `~/.codex/AGENTS.md` | 8/8 | 3 | 5 |
| Claude Code, `~/.claude/CLAUDE.md` の `@import` 行 1 つ | 7/8 | 3 | 4 |
| gajae-code, `~/.gjc/agent/AGENTS.md` | 7/8 | 3 | 4 |
| OpenCode, `~/.config/opencode/AGENTS.md` | 6/8 | 2 | 5 |
| Kiro CLI, `~/.kiro/steering/xout.md` | 6/8 | 2 | 4 |

どのツールも自分のファイルを読んでいました。どの行でも、ブロックがそこにあったという理由で少なくとも 2 つの答えが変わっています。不一致のほうも読む価値があります。Claude が外した 1 件は、またしてもテストを先に書く癖でした。gajae-code と Kiro は、バグ修正で先に提案せずそのまま手を動かしました。Kiro はエラーが出ても自力で直そうとし続けました。OpenCode の 2 件は、選択肢の記号が入っていない長い散文の回答が 1 件と、テストを先に書く癖が 1 件です。Kiro のドキュメントは、グローバルな steering は IDE 用、ワークスペースの steering は CLI 用と説明していますが、ここでは CLI がグローバルのファイルを読みました。プローブは、こういうものを見つけるためにあります。
...
- **それらのファイルを読むのは、既定ではパターンマッチです。** 依存関係はなく、オフラインで動き、当たった行にはそれを捉えたパターンが必ず添えられます。パターンの精度は、4 言語のルール 276 行にラベルを付けたコーパスで測っています (適合率 1.00、いちばん弱い軸で再現率 0.96。どちらかが落ちればテストが失敗します)。それでもコーパスの外の言い回しは取りこぼします。そこで `xout mine --runner "claude -p --output-format text"` (下の表のランナーならどれでも) が同じ行を手元のエージェントに渡し、行ごとにどの軸のどの値を述べているのかを答えさせます。2 つの層は突き合わされ、生の回答はレシートにすべて残ります。これを書いた本人の 187 行の `~/.claude/CLAUDE.md` では、両者が一致したのが 8 行、パターンが取りこぼしていた 5 行をエージェントが見つけ (「Minimal change principle」は範囲、「No unsolicited docstrings, comments, or type annotations」はコメント)、そもそも好みではなかったパターンの当たりを 10 件落としました。`xout conflicts --runner …` も同じフラグを受け付けます。付けなければ、何も変わりません。
...
- **`xout reconcile`** は、古いファイルの中でいまや `XOUT.md` と重複している行を一覧し、提案パッチを `~/.claude/xout/reconcile/` 配下に書き出します。重複行を実際に取り除くのは `xout reconcile --apply --grant` だけで、それもセーブポイントを取ってからです。自分のルールと似て読めるだけの行は、類似度スコアを添えて別枠に並べるだけで、取り除くことはありません。
...
セッションが測るのは 8 つの軸です。すでに書いてある `CLAUDE.md` には、それよりずっと多くの行が入っていて、その大半は、何かが変わるかどうかをエージェントの前に置いて確かめたことがありません。`xout audit` は、その行を 1 本ずつ測ります。

行ごとに、手元のエージェントに小さな場面を書かせます。やること 1 つ、その行に従う次の行動 1 つ、その行を破る次の行動 1 つ。そしてその場面を 2 回投げます。1 回はそのまま、1 回はその 1 行だけを常設の指示として前に置いて。返ってきた答えで、その行は 4 つのどれかに分かれます:

- **もともとそうする** - その行がなくてもエージェントはそちらへ行きます。文が何もしていないので、消しても何も変わりません。
- **その行が効いている** - そのまま問うと逆へ行き、その行があると従います。ルールとは、こうあるべきものです。
- **無視された** - 行はすぐ目の前にあるのに、エージェントは逆をやります。これがいちばん重要です。守ってくれるように読めて、守っていません。
- **判別できない** - 読める答えがない、あるいは試行が割れました。
...
見出し、表の行、コードブロック、一語だけの行は、送る前に落とします。残りは `--limit` (既定は 60) で頭打ちにするので、最初の 1 回は安く済みます。最初の結果を見てから上げてください。

これを書いた本人の 187 行のグローバルな `CLAUDE.md` を、`claude -p` をランナーにして最初に回した記録です。36 行は見出しやコードとして落ち、先頭の 60 行が送られ、エージェントはそのうち 57 行に場面を書きました (残りの 3 行は指示ではないと判断しています)。その 57 行のうち、55 行はもともとそうする、2 行はその行が効いている、無視されたが 0、判別できないが 0 でした。何か月もかけて手で書いたルールファイルでも、そのほとんどはエージェントがどのみちやることで、この点検はどの 2 文が自分の場所を稼いでいるのかを教えてくれます。役に立ったのは矛盾のほうでした。4 組出てきて、その中に "always respond in Korean" と "ASCII output only" (ハングルは ASCII ではありません)、そして "always end with a three-line summary" と "explain only when it is not obvious" がありました。どちらも誰も気づいていませんでした。レシートには、場面も回答もすべて入っています。
...
8 つの軸で、あなたが気にかけていることが全部おさまるわけではありません。そのためにカタログが増えることもありません。残りは自分で書いてください: `xout own add "ローカルのシェルから本番データベースに触らないこと。"` この一文は `XOUT.md` の末尾に自分のセクションを持って着地します。打ったとおり、一字一句そのままで。xout はこの行を測りも、順位づけも、書き換えもしません。`xout own list` は自分の行を id つきで並べ、`xout own drop <id>` は 1 本を取り下げます。どちらもその場で `XOUT.md` を着地し直します。これらの行は X と同じ追記専用の台帳に入るので、消えるものは何もありません。取り下げれば墓標が 1 つ追記され、次の着地でたたみ込まれます。
...
| `xout mine --runner "…"` | 同じことを、手元のエージェントが行ごとに判定し、その結果をパターンの側と突き合わせる | 自分のディレクトリ (`judgments/`) | オプトイン |
| `xout conflicts [paths] [--runner "…"]` | ルールファイルの中で、自分のルールと逆のことを言っている行を一覧する | なし (`--runner` ならレシート) | - |
...
| `xout probe --repeat N` | 問いごとに N 回投げ、多数決で決める。回答はすべて残す | 自分のディレクトリ (`probes/`) | オプトイン |
| `xout probe --context-file FILE` | 実際のプロジェクト文書をルールの前に置き、埋もれても生き延びるかを見る | 自分のディレクトリ (`probes/`) | オプトイン |
| `xout probe --via-target ID` | ルールをプロンプトから外し、そのツール自身のルールファイルからブロックを抜いて 1 回、戻して 1 回問う | そのツールのファイル (先にセーブポイント)、最後に元へ戻す | オプトイン |
| `xout audit [paths] --runner "…"` | すでにあるルールファイルの行を 1 本ずつ測る: エージェントはどのみちそうするのか、その行が答えを変えるのか、それとも無視されるのか | 自分のディレクトリ (`audits/`) | オプトイン |
| `xout own add "…"` / `list` / `drop <id>` | 8 本のルールの隣に置く、自分の言葉で書いた自分の行 | 自分のディレクトリのみ | - |
