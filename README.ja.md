...
きれいに揃わなかった 2 件は、どちらもバグ修正の場面です。Codex は一度、「1 回だけ再試行して、あとは報告する」というルールを越えて再試行を続けました。OpenCode は一度、失敗するテストを先に書きました。それ以外は 3 回とも維持されています。
...
どのツールも自分のファイルを読んでいました。どの行でも、ブロックがそこにあったという理由で少なくとも 2 つの答えが変わっています。不一致のほうも読む価値があります。gajae-code と Kiro は、バグ修正で先に提案せずそのまま手を動かしました。Kiro はエラーが出ても自力で直そうとし続けました。OpenCode の 2 件は、選択肢の記号が入っていない長い散文の回答が 1 件と、テストを先に書く癖が 1 件です。Kiro のドキュメントは、グローバルな steering は IDE 用、ワークスペースの steering は CLI 用と説明していますが、ここでは CLI がグローバルのファイルを読みました。プローブは、こういうものを見つけるためにあります。
