...
<sub>이 녹화는 목업이 아니라 실제 세션입니다: 화면의 모든 페어와 규칙은 실제 엔진이 만든 것입니다 (`scripts/record_tui_demo.py`가 라이브 `ColdOpenSession`을 구동).</sub>
...
|  | 손으로 쓴 CLAUDE.md / 인터뷰형 도구 | xout |
|---|---|---|
| 선호를 얻는 방법 | 직접 진술 - 규범 압력이 진술을 왜곡 | 선택으로 드러남: 구체적인 두 행동 중 아닌 쪽에 X |
| 규칙의 출처 | 기억으로 작성, 출처 없음 | 긋기에서 컴파일 - `xout why`가 모든 규칙을 증거로 소급 |
| 일상 vs 고위험 작업 | 규칙 하나로 모든 상황 커버 | 당신의 X가 실제로 갈렸을 때만 일상/고위험 경계에서 규칙이 분기 |
| 되돌리기 | 파일을 손으로 고치고 기도 | 영수증으로 증명되는 한 줄; `xout undo`가 정확히 그것만 제거 |
| **규칙이 낡으면** | 에이전트가 다시 거슬릴 때까지 조용히 드리프트 | 낡은 규칙은 2분 세션에서 다시 긋고, 불안정한 규칙은 재심 대기열에 표시 |
